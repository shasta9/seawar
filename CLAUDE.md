...
- **Map file format:** The server loads a map file at startup. Format TBD but should
  be simple and human-readable (JSON or a custom text format).
- **Procedural generation:** A map generator tool should support controllable parameters,
  for example:
  - Coastline shape and island density
  - Number and severity of choke points (narrow sea passages)
  - Port placement (coast locations, separation distance between opposing ports)
  - Merchant ship count and positions
  - Starting positions for sub and destroyer
...
- **Phase:** Map tool can be developed in parallel with or after the core game.
